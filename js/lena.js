...
LenaJS.canny = function(pixels) {
  ...
  var deltaY = LenaJS.sobelHorizontal(pixels)
  var deltaX = LenaJS.sobelVertical(pixels)
  ...
  return LenaJS.thresholding(pixels, 5, 130)
}
...
LenaJS.thresholding = function(pixels, lv, rv) {
  for (var i = 0; i < pixels.data.length; i += 4) {
    var r = pixels.data[i],
      g = pixels.data[i+1],
      b = pixels.data[i+2]
    ...
    pixels.data[i] = pixels.data[i+1] = pixels.data[i+2] = v > thrl ? (v < thrv ? 255 : 0) : 0
  }
