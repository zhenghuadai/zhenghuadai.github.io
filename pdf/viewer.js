...
function webViewerInitialized() {
  ...
  document.getElementById('htmlexport').addEventListener('click', htmlExporter);
  document.getElementById('secondaryHtmlexport').addEventListener('click', htmlExporter);
...
var htmlExporter = (function(){
  ...
  var downloadFile = function (filename, html) {
      var pdfviewerdiv = document.getElementById("viewer");
      var dataValue = "<html><head><meta charset='utf-8'/><LINK REL='stylesheet' TYPE='text/css' HREF='http://www.biosuefi.com/pdf/pdf.css'></head><body>"
          + html 
          + "</body></html>";
    ...
    if (BlobBuilder == undefined) {
        var bb = new Blob([dataValue], { 'type': 'text/html'});
  ...
  function htmlExport(){
      ...
      var pdfhtml = "<div id='viewer' class='pdfViewer' >";
      var pdfViewer = PDFViewerApplication.pdfViewer;
      var width = 1019, height=1319;
      function loadPage(index){
          pdfDocument.getPage(index).then(function (page) {
              page.getTextContent().then(function (textContent) {
                  ...
                  var pageView = PDFViewerApplication.pdfViewer.getPageView(index-1);
                  pageView.draw();
                  pageView.textLayer.setTextContent(textContent);
                  pageView.textLayer.renderLayer();
                  pdfhtml += "<div id='pageContainer" + index + "' class='page' style='width:" + width + "px; height:" + height + "px;' data-page-number='" + index + "' data-loaded='true'>" 
                         + getHTML(pageView.textLayer.textLayerDiv)
                         + "</div>";
                  if(index == pages ){
                      pdfhtml += '</div>';
                      downloadFile(document.title+".html", pdfhtml);
                  }
              });
          });
      }
